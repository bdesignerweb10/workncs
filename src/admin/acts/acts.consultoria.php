	...
	if(isset($_GET['act']) && !empty($_GET['act'])) {
		switch ($_GET['act']) {
				...
				try {
					...
					if(isset($_POST) && !empty($_POST) && $_POST["titulo"]) {
						...
						if(!isset($_POST["titulo"]) || empty($_POST["titulo"])) {
							$errMsg .= "Titulo (Título da consultoria (Serviço))";
							$isValid = false;
						}
						...
						if(!$isValid) {
							...
							/*if(isset($_FILES['img'])) {
							    ...
							    $pathImagem = "../../img/servicos/" . ($_FILES['img']['name']);
							...
							if ($conn->query($qry_consultoria) === TRUE) {
						        ...
						        throw new Exception("Erro ao inserir o evento: " . $qry_consultoria . "<br>" . $conn->error);
							}							
						}
					}
					else {
					...
					if(isset($_POST) && !empty($_POST) && $_POST["titulo"]) {
						...
						if(!$isValid) {
							...
							if ($conn->query($qry_consultoria) === TRUE) {
						        ...
						        throw new Exception("Erro ao alterar o evento: " . $qry_consultoria . "<br>" . $conn->error);
							}
						}
					}
					else {
			...
			try {
				...
				$qrydel_consultoria = "DELETE FROM consultoria WHERE id = $id";
				if ($conn->query($qrydel_consultoria) === TRUE) {
				
					$qrydelconsultoria = "DELETE FROM consultoria WHERE id = $id";
					if ($conn->query($qrydelconsultoria) === TRUE) {
						$conn->commit();
						echo '{"succeed": true}';
					} else {
				        throw new Exception("Erro ao remover consultoria: " . $qrydelconsultoria . "<br>" . $conn->error);
					}
				} else {
			        throw new Exception("Erro ao remover consultoria: " . $qrydel_consultoria . "<br>" . $conn->error);
				}
			} catch(Exception $e) {
				$conn->rollback();
